...
L1_WORDS = [
    ("おはよう", []), ("パソコン", ["ぱそこん"]), ("電話", ["でんわ"]),  # 追加: ひらがな入力許容
]
L2_SENTENCES = [
    ("今から帰る", ["いまからかえる"]),  # 追加: ひらがな入力許容
    ("気をつけてね", ["気を付けて", "きをつけてね"]),  # 追加: 表記ゆれ + ひらがな入力許容
    ("お腹がすいた", ["お腹が空いた", "おなかがすいた"]),  # 追加: 表記ゆれ + ひらがな入力許容
]
L3_SENTENCES = [
    ("来週はパソコンを買いに行く", ["来週はパソコンを、買いに行く", "らいしゅうはぱそこんをかいにいく"]),  # 追加: 読点あり + ひらがな入力許容
    ("子供の病院に行こう", ["こどものびょういんにいこう"]),  # 追加: ひらがな入力許容
    ("車で行こうか、電車で行こうか", ["車で行こうか電車で行こうか", "くるまでいこうかでんしゃでいこうか"]),  # 追加: 読点なし + ひらがな入力許容
]
